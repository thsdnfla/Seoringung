...
const reviewDialog=document.querySelector('#review-dialog');document.querySelector('#open-review').addEventListener('click',()=>reviewDialog.showModal());
document.querySelector('#review-form').addEventListener('submit',event=>{event.preventDefault();const data=new FormData(event.target);const article=document.createElement('article');article.className='review-card';article.innerHTML=`<div>${data.get('rating')} <time>${new Date().getFullYear()}.${pad(new Date().getMonth()+1)}</time></div><p></p><small></small>`;article.querySelector('p').textContent=data.get('content');article.querySelector('small').textContent=data.get('author')||'익명';document.querySelector('#review-grid').prepend(article);reviewDialog.close();event.target.reset()});
